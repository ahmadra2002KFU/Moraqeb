...
export function buildIntelligenceContext(v2Data, deltaData) {
  ...
  const sections = [];
  const ts = v2Data.meta?.timestamp || new Date().toISOString();
  sections.push(`=== MORAQEB INTELLIGENCE SNAPSHOT ===`);
  sections.push(`Timestamp: ${ts}`);
  sections.push(`Sources: ${v2Data.meta?.sourcesOk || 0}/${v2Data.meta?.sourcesQueried || 0} operational`);
  sections.push('');
  ...
  if (v2Data.fred?.length) {
    ...
    for (const f of key) {
      ...
      sections.push(`  ${f.id}: ${f.value}${change}`);
    }
    // Yield curve status
  ...
  if (v2Data.treasury) {
    ...
    sections.push(`  Total National Debt: $${v2Data.treasury}T`);
    sections.push('');
  }
  ...
  if (v2Data.markets?.length) {
    sections.push('--- MARKETS ---');
    for (const m of v2Data.markets.slice(0, 15)) {
  ...
  if (v2Data.tg) {
    ...
    if (v2Data.tg.urgent?.length) {
      ...
      for (const p of v2Data.tg.urgent.slice(0, 8)) {
        sections.push(`    - ${(p.text || '').substring(0, 200)}`);
      }
    }
    sections.push('');
  }
  ...
  if (v2Data.nuke?.length) {
    const anomalies = v2Data.nuke.filter(n => n.anom);
    if (anomalies.length) {
      sections.push('--- NUCLEAR ANOMALIES ---');
      for (const n of anomalies) {
        sections.push(`  ${n.site}: ${n.cpm} CPM (anomaly detected)`);
      }
      sections.push('');
    }
  }
  ...
  if (v2Data.acled) {
    ...
    sections.push(`  Total events (7-day): ${v2Data.acled.total || 0}`);
    sections.push(`  Fatalities: ${v2Data.acled.fatalities || 0}`);
    if (v2Data.acled.deadliest?.length) {
      sections.push('  Deadliest events:');
      for (const e of v2Data.acled.deadliest.slice(0, 5)) {
        sections.push(`    - ${e.country}: ${e.event_type} — ${e.fatalities} killed`);
      }
    }
    sections.push('');
  }
  ...
  if (v2Data.defense?.length) {
    ...
    for (const d of v2Data.defense.slice(0, 5)) {
      sections.push(`  - $${((d.amount || 0) / 1e6).toFixed(0)}M to ${d.recipient}: ${d.description?.substring(0, 100) || ''}`);
    }
    sections.push('');
  }
  ...
  if (v2Data.space) {
    sections.push('--- SPACE ---');
    if (v2Data.space.launches) sections.push(`  Recent launches: ${v2Data.space.launches}`);
  ...
  if (v2Data.news?.length) {
    ...
    for (const n of v2Data.news.slice(0, 10)) {
  ...
  if (deltaData?.summary) {
    ...
    if (deltaData.signals?.escalated?.length) {
      ...
      for (const s of deltaData.signals.escalated) {
        sections.push(`    - ${s.label}: ${s.previous} → ${s.current} (${(s.changePct || 0) > 0 ? '+' : ''}${(s.changePct || 0).toFixed(1)}%)`);
      }
    }
