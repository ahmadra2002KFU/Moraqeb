...
export function buildIntelligenceContext(v2Data, deltaData) {
  ...
  if (v2Data.thermal?.length) {
    ...
    if (hotRegions.length) {
      ...
      if (v2Data.thermalMeta?.uniqueDetections != null) {
        sections.push(`  Deduplicated monitored coverage: ${v2Data.thermalMeta.uniqueDetections} unique detections (${v2Data.thermalMeta.duplicateMemberships || 0} overlapping memberships removed); observed ${v2Data.thermalMeta.observedAt || 'unknown'}; cause unverified.`);
      }
