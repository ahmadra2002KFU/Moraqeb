...
const DEFAULT_COUNT_THRESHOLDS = {
  ...
  thermal_new: 25,         // new unique observations in compatible 24h windows
  thermal_new_high: 10,    // new high-confidence observations
...
const COUNT_METRICS = [
  ...
  { key: 'air_total', extract: d => d.airMeta?.available === false ? null : d.air?.reduce((s, a) => s + a.total, 0) ?? null, label: 'Air Activity' },
  { key: 'who_alerts', extract: d => d.who?.length || 0, label: 'WHO Alerts' },
  { key: 'conflict_events', extract: d => d.acled?.available === false ? null : d.acled?.totalEvents, label: 'Conflict Events' },
  { key: 'conflict_fatalities', extract: d => d.acled?.available === false ? null : d.acled?.totalFatalities, label: 'Conflict Fatalities' },
...
const RISK_KEYS = ['vix', 'hy_spread', 'urgent_posts', 'conflict_events', 'thermal_activity_update'];
...
export function computeDelta(current, previous, thresholdOverrides = {}) {
  ...
  // FIRMS uses set-based rolling-window semantics. Expiration from the 24-hour
  // window is never interpreted as de-escalation or resolution.
  const currentThermal = current.thermalMeta || {};
  const previousThermal = previous.thermalMeta || {};
  const compatibleThermal = currentThermal.available === true
    && previousThermal.available === true
    && currentThermal.stale !== true
    && previousThermal.stale !== true
    && currentThermal.coverage?.complete !== false
    && previousThermal.coverage?.complete !== false
    && currentThermal.product
    && currentThermal.product === previousThermal.product
    && currentThermal.windowHours === previousThermal.windowHours
    && Array.isArray(currentThermal.observationIds)
    && Array.isArray(previousThermal.observationIds);
  if (compatibleThermal) {
    const previousIds = new Set(previousThermal.observationIds);
    const previousHigh = new Set(previousThermal.highConfidenceObservationIds || []);
    const newlyObserved = currentThermal.observationIds.filter(id => !previousIds.has(id));
    const newHighConfidence = (currentThermal.highConfidenceObservationIds || []).filter(id => !previousHigh.has(id));
    const material = newlyObserved.length >= (cntThresholds.thermal_new ?? 25)
      || newHighConfidence.length >= (cntThresholds.thermal_new_high ?? 10);
    if (material) {
      signals.new.push({
        key: 'thermal_activity_update',
        label: 'New Unique Thermal Observations',
        newlyObserved: newlyObserved.length,
        newHighConfidence: newHighConfidence.length,
        direction: 'new-observations',
        severity: newHighConfidence.length >= 20 || newlyObserved.length >= 100 ? 'high' : 'moderate',
      });
    } else {
      signals.unchanged.push('thermal_activity_update');
    }
  }
  ...
  const seenHashes = new Set(
    (previous.tg?.urgent || []).map(p => p.hash || contentHash(p.text)).filter(Boolean)
  );

  for (const post of (current.tg?.urgent || [])) {
    const hash = post.hash || contentHash(post.text);
  ...
  const material = [...signals.new, ...signals.escalated, ...signals.deescalated];
  const criticalCount = material.filter(signal => signal.severity === 'critical').length;
  return {
