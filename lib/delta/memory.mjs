...
import { computeDelta, contentHash } from './engine.mjs';
...
export class MemoryManager {
  ...
  addRun(synthesizedData) {
    ...
    const delta = computeDelta(synthesizedData, previous, this.thresholds);
  ...
  _compactForStorage(data) {
    return {
      ...
      gscpi: data.gscpi,
      tg: { posts: data.tg?.posts, urgent: (data.tg?.urgent || []).map(p => ({
        text: p.text?.substring(0, 200), hash: p.hash || contentHash(p.text), channel: p.channel,
        date: p.date, postId: p.postId, url: p.url,
      })) },
      thermal: (data.thermal || []).map(t => ({ region: t.region, det: t.det, night: t.night, hc: t.hc, observedAt: t.observedAt || null, stale: Boolean(t.stale) })),
      thermalMeta: {
        available: data.thermalMeta?.available !== false,
        status: data.thermalMeta?.status || null,
        uniqueDetections: data.thermalMeta?.uniqueDetections ?? null,
        duplicateMemberships: data.thermalMeta?.duplicateMemberships ?? null,
        highConfidence: data.thermalMeta?.highConfidence ?? null,
        nightDetections: data.thermalMeta?.nightDetections ?? null,
        observationIds: data.thermalMeta?.observationIds || [],
        highConfidenceObservationIds: data.thermalMeta?.highConfidenceObservationIds || [],
        product: data.thermalMeta?.product || null,
        windowHours: data.thermalMeta?.windowHours || null,
        coverage: data.thermalMeta?.coverage || null,
        observedAt: data.thermalMeta?.observedAt || null,
        stale: Boolean(data.thermalMeta?.stale),
      },
      air: (data.air || []).map(a => ({ region: a.region, total: a.total })),
      airMeta: { available: data.airMeta?.available !== false, status: data.airMeta?.status || null },
      nuke: (data.nuke || []).map(n => ({ site: n.site, anom: n.anom, cpm: n.cpm, stale: n.stale, lastReading: n.lastReading })),
      who: (data.who || []).map(w => ({ title: w.title })),
      acled: { available: data.acled?.available !== false, status: data.acled?.status || null, totalEvents: data.acled?.totalEvents, totalFatalities: data.acled?.totalFatalities },
      ...
      health: (data.health || []).map(source => ({ n: source.n, status: source.status, err: source.err, stale: source.stale })),
    };
  }
