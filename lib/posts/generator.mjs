...
export async function generatePost(apiKey, model, v2Data, deltaData, recentTopics = [], dedupCheck = null) {
  ...
  // ── Generate EN with retry ────────────────────────────────────────────────
  let enResult, arResult;
  let totalInput = 0, totalOutput = 0;

  enResult = await Promise.allSettled([callMiniMaxWithRetry(apiKey, model, enPrompt)]);
  enResult = enResult[0];

  let enText = enResult.status === 'fulfilled' ? stripThinking(enResult.value.text) : null;
  if (enResult.status === 'fulfilled') {
    totalInput += enResult.value.inputTokens || 0;
    totalOutput += enResult.value.outputTokens || 0;
  }
  ...
  // ── Generate AR with retry ────────────────────────────────────────────────
  arResult = (await Promise.allSettled([callMiniMaxWithRetry(apiKey, model, arPrompt)]))[0];
